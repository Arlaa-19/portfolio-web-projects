...
$sql    = "SELECT MAX(id_kriteria) AS maxid FROM tabel_kriteria";
...
$query = "SELECT MAX(kriteria) AS max_kode FROM tabel_kriteria";
            ...
            <div class="container">
                <div class="row d-flex flex-column flex-lg-row text-center text-md-start">
                    <div class="col-lg-6 mb-3">
                        <img src="../asset/img/PNG/tambahK.png" alt="" class="img-fluid">
                    </div>
                    <div class="col-lg-6">
                        <div class="subContainer mycard">
                            <h3 class="sacramento-regular">Tambah Kriteria .</h3>
                            <form action="" method="post" class="row formTambah">
                                <div class="col">
                                    <div class="row">
                                        <div class="col">
                                            <label for="inputIdKriteria" class="form-label">Id Kriteria</label>
                                            <input type="text" name="id_kriteria" class="form-control" id="inputIdKriteria" placeholder="Id Kriteria" aria-label="First name" value="<?= $kodeoto ?>" readonly required>
                                        </div>
                                        <div class="col">
                                            <label for="inputKriteria" class="form-label">Kriteria</label>
                                            <input type="text" name="kriteria" class="form-control" id="inputKriteria" placeholder="Kriteria" aria-label="Last name" value="<?= $kodeOtomatis ?>" readonly required>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12">
                                    <label for="inputKeterangan" class="form-label">Keterangan</label>
                                    <input type="text" name="keterangan" class="form-control" id="inputKeterangan" placeholder="input keterangan kriteria" required>
                                </div>
                                <div class="col-12">
                                    <label for="inputnumber" class="form-label">Bobot Kriteria</label>
                                    <input type="number" name="bobot" min="0" step="0.1" class="form-control" id="inputnumber" placeholder="input bobot" required>
                                </div>
                                <div class="col-12">
                                    <div class="operasi">
...
if (isset($_POST['simpan'])) {
    ...
    $sql_kriteria   = "SELECT * FROM tabel_kriteria WHERE keterangan = '$keterangan'";
    $query_kriteria = $conn->query($sql_kriteria);
    $cek_kriteria   = $query_kriteria->num_rows;

    if ($cek_kriteria > 0) {
        echo "<script>alert('Data Sudah Ada!') </script>";
        echo "<script>window.location.href = \"tambahKriteria.php\" </script>";
    } else {
        // Tambah data ke tabel_kriteria
        $query = "INSERT INTO tabel_kriteria VALUES ('$id_kriteria','$kriteria', '$keterangan', '$bobot')";
        ...
        if ($tambah == true) {
            ...
            $alter_query = "ALTER TABLE tabel_alternatif ADD $keterangan VARCHAR(255)";
