...
if (isset(($_POST['start_date'])) && isset($_POST['end_date'])) {
    ...
    $queryAlternatif = "SELECT id_alternatif, tanggal, nama_kelompok, kecamatan, kelurahan, ";

    // Debug: Memastikan data kriteriaList tersedia
    echo "<pre>";
    print_r($kriteriaList);
    echo "</pre>";

    $kriteriaListNew = array();
    foreach ($kriteriaList as $kriteria) {
        // Validasi dan modifikasi nama kolom
        if (isset($kriteria['keterangan'])) {
            $keterangan = $kriteria['keterangan'];
            $keterangan = preg_replace('/[^a-zA-Z0-9_]/', '_', $keterangan); // Mengganti karakter selain alfanumerik dan underscore dengan underscore
            $kriteriaListNew[] = $keterangan;
            $queryAlternatif .= $keterangan  . ", ";
        } else {
            // Debug: Menampilkan pesan jika keterangan tidak ditemukan
            echo "Kunci 'keterangan' tidak ditemukan dalam array kriteria: ";
            print_r($kriteria);
        }
    }

    $queryAlternatif = rtrim($queryAlternatif, ", "); // Menghapus koma terakhir
    $queryAlternatif .= " FROM tabel_alternatif";

    // Debug: Menampilkan query yang dibentuk
    echo "<pre>";
    echo $queryAlternatif;
    echo "</pre>";

    // Debug: Memastikan data kriteriaList tersedia
    echo "<pre>";
    print_r($kriteriaListNew);
    echo "</pre>";
                        ...
                        <div class="container overflow-auto">
                            <table id="example" class="table table-responsive table-striped table-hover table-bordered">
                                <thead class="bgtosca">
                                    <tr>
                                        ...
                                        <?php foreach ($kriteriaList as $kriteria) : ?>
                                            <th class="text-start text-light"><?= $kriteria['keterangan']; ?></th>
                                        <?php endforeach; ?>
                                ...
                                <tbody class="bg-white">
                                    <?php
                                    // Loop melalui data alternatif dan tampilkan setiap baris
                                    while ($row = mysqli_fetch_assoc($resultAlternatif)) {
                                        echo '<tr>';
                                        echo '<td class="text-start">' . $row['tanggal'] . '</td>';
                                        echo '<td class="text-start">' . $row['nama_kelompok'] . '</td>';
                                        echo '<td class="text-start">' . $row['kecamatan'] . '</td>';
                                        echo '<td class="text-start">' . $row['kelurahan'] . '</td>';
                                        foreach ($kriteriaListNew as $kriteria) {
                                            echo '<td class="text-start">' . $row[$kriteria] . '</td>';
                                        }
                                        echo "
                                            <td class='text-start'>
                                                <div class='iconAksi'>
